...
func (s *State) activate(index int) tea.Cmd {
	...
	if active, ok := s.Active(); ok {
		if leaf, ok := active.(Leaf); ok {
			cmds = append(cmds, leaf.Deactivate())
		}
	}
	s.activeIndex = index
	...
	if active, ok := s.Active(); ok {
		if leaf, ok := active.(Leaf); ok {
			cmds = append(cmds, leaf.Activate())
		}
	}

	cmds = append(cmds, soda.Redraw)
...
func (s *State) Init(ctx context.Context) tea.Cmd {
	return tea.Sequence(s.branch.Init(ctx), s.activate(0))
}
